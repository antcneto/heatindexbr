...
(RH, %) into a single thermal comfort index, building on the
foundational work of Steadman (1979) on human physiological response to
heat and humidity. Wind speed is not considered, which makes the index
tractable for regional-scale mapping where homogeneous wind data are
unavailable. Four synoptic hours were selected (00h, 09h, 15h, and 21h
local time, UTC-3) to capture the full thermal cycle of a typical day:
nocturnal minimum, morning warming, afternoon peak, and evening cooling.
...
completeness threshold of 75% was applied, resulting in 59 to 73
stations per hour depending on data availability. The 15h hour (18h UTC)
achieves the largest network: 73 stations.
...
was tested and rejected: it introduced a systematic bias of +0.246°C at
15h relative to direct IC interpolation. The direct approach is used
throughout.
...
Gaussian) converged to a pure nugget effect: Sill = 0, Nugget
approximately 1.05. This means no residual spatial autocorrelation
remains after fitting, indicating the model has absorbed all systematic
spatial structure.
...
Simple correlations for comparison: IC × ERA5 = 0.933, IC × altitude =
-0.778, altitude × ERA5 = -0.529. The raster is not redundant with an
elevation model, as ERA5 explains nearly all remaining variance after
altitude is controlled for, confirming the thermodynamic basis of the
interpolation.
...
neighbour station is -0.109 (non-significant), meaning prediction error
does not grow with distance.

The largest errors occur in the 33 to 55 km range, corresponding to
coastal and Agreste zones with steep gradients. Interior Semiarid areas
show smaller errors because of genuine climatic homogeneity, not because
stations are closer. Consequently, the distance-to-station raster should
be interpreted as a descriptive map of the monitoring network, not as a
...
ROTHFUSZ, L. P. The heat index equation (or, more than you ever wanted
to know about heat index). NWS Technical Attachment SR 90-23. National
Weather Service, Fort Worth, TX, 1990.

STEADMAN, R. G. The assessment of sultriness. Part I: a
temperature-humidity index based on human physiology and clothing
science. **Journal of Applied Meteorology**, v. 18, n. 7, p. 861-873,
1979.

BRASIL. Resolução CONDEL/SUDENE n. 176, de 26 de março de 2024. Aprova a
delimitação do Semiárido Brasileiro. Superintendência do Desenvolvimento
do Nordeste, Recife, 2024. Disponível em:
